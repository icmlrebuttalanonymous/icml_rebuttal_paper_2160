...
def erk_sparsity(args, model, device=torch.device("cuda:0")):
    if "opt" in args.model:
        blocks = model.model.decoder.layers    
    else:
        blocks = model.model.layers
    
    amount = args.sparsity_ratio
    
    layers = [find_layers(blocks)]
    unmaskeds = []
    for layer in layers:
        for name in layer:
            unmaskeds.append(layer[name].weight.numel())
    unmaskeds = torch.tensor(unmaskeds)
        
    wlist = get_weights(blocks)
    ...
    while is_eps_invalid:
        ...
        ers_of_prunables = erks*(1.0-layers_to_keep_dense)
...
def erk_sparsity_plus_plus(args, model, device=torch.device("cuda:0")):
    if "opt" in args.model:
        blocks = model.model.decoder.layers[:-1]    
    else:
        blocks = model.model.layers[:-1]
    
    amount = args.sparsity_ratio * (len(blocks) + 1) / (len(blocks))
    ...
    amounts = torch.zeros(num_layers+7)
    ...
    for i in range(num_layers, num_layers+7):
        amounts[i] = 0.0

    print(amounts)
    return amounts

def erk_sparsity_plus(args, model, device=torch.device("cuda:0")):
    if "opt" in args.model:
        blocks = model.model.decoder.layers    
    else:
        blocks = model.model.layers
    ...
    amount = args.sparsity_ratio * torch.sum(unmaskeds) / torch.sum(unmaskeds[:-1])
    
    unmaskeds = unmaskeds[:-1]
    wlist = get_weights(blocks)
    wlist = wlist[:-1]
    erks = torch.zeros(len(wlist))
    for idx,w in enumerate(wlist):
    ...
    amounts = torch.zeros(num_layers+1)
...
###############################################################################################################
def prune_magnitude_ww_plus(args, model, tokenizer, device=torch.device("cuda:0"), prune_n=0, prune_m=0):
    # calculate ww-based layer
    all_layer_ratio = ww_sparsity_plus(args, model, device)
    # magnitude pruning
    prune_magnitude(args, model, tokenizer, device, ratios=all_layer_ratio)
    

def prune_wanda_ww_plus(args, model, tokenizer, device=torch.device("cuda:0"), prune_n=0, prune_m=0):
    all_layer_ratio = ww_sparsity_plus(args, model, device)

    # wanda pruning
    prune_wanda(args, model, tokenizer, device, ratios=all_layer_ratio)
    
def prune_sparsegpt_ww_plus(args, model, tokenizer, device=torch.device("cuda:0"), prune_n=0, prune_m=0):
    all_layer_ratio = ww_sparsity_plus(args, model, device)
...
def prune_magnitude_ww_inverse(args, model, tokenizer, device=torch.device("cuda:0"), prune_n=0, prune_m=0):
    s1 = 1.0 - args.epsilon
    s2 = 1.0 + args.epsilon
    
    # calculate ww-based layer
    all_layer_ratio = ww_sparsity_inverse(args, model, device, s1, s2)
    # magnitude pruning
    prune_magnitude(args, model, tokenizer, device, ratios=all_layer_ratio)
    

def prune_wanda_ww_inverse(args, model, tokenizer, device=torch.device("cuda:0"), prune_n=0, prune_m=0):
    s1 = 1.0 - args.epsilon
    s2 = 1.0 + args.epsilon
    all_layer_ratio = ww_sparsity_inverse(args, model, device, s1, s2)
...
def prune_sparsegpt_ww_inverse(args, model, tokenizer, device=torch.device("cuda:0"), prune_n=0, prune_m=0):
    s1 = 1.0 - args.epsilon
    s2 = 1.0 + args.epsilon
    all_layer_ratio = ww_sparsity_inverse(args, model, device, s1, s2)
...
def prune_magnitude_global(args, model, tokenizer, device=torch.device("cuda:0"), prune_n=0, prune_m=0, ratios=None):
    if "OPT" in model.__class__.__name__:
        layers = model.model.decoder.layers
    else:    
        layers = model.model.layers
    ...
    for i in range(len(layers)):
        ...
        for name in subset:
            W = subset[name].weight.data 
            W_metric = torch.abs(W)
            W_metric_all.append(W_metric.flatten())
            num += W.numel()
    print(num)
    W_metric_all = torch.cat(W_metric_all)
    thresh = np.sort(W_metric_all.cpu().numpy())[int(num * args.sparsity_ratio)]
    thresh = torch.tensor(thresh).cpu()
    # pruning
    for i in range(len(layers)):
        layer = layers[i]
        subset = find_layers(layer)
...
def prune_wanda_global(args, model, tokenizer, device=torch.device("cuda:0"), prune_n=0, prune_m=0, ratios=None):
    use_cache = model.config.use_cache 
    model.config.use_cache = False 
...
def prune_magnitude_erk(args, model, tokenizer, device=torch.device("cuda:0"), prune_n=0, prune_m=0):
    # calculate ww-based layer
    all_layer_ratio = erk_sparsity(args, model, device)
    # magnitude pruning
    print(all_layer_ratio)
    prune_magnitude(args, model, tokenizer, device, ratios=all_layer_ratio)
    
def prune_magnitude_erk_plus(args, model, tokenizer, device=torch.device("cuda:0"), prune_n=0, prune_m=0):
    # calculate ww-based layer
    all_layer_ratio = erk_sparsity_plus(args, model, device)
    # magnitude pruning
    prune_magnitude(args, model, tokenizer, device, ratios=all_layer_ratio)
    
def prune_magnitude_erk_plus_plus(args, model, tokenizer, device=torch.device("cuda:0"), prune_n=0, prune_m=0):
    # calculate ww-based layer
    all_layer_ratio = erk_sparsity_plus_plus(args, model, device)
    # magnitude pruning
    prune_magnitude(args, model, tokenizer, device, ratios=all_layer_ratio)

def prune_magnitude_uniform_plus(args, model, tokenizer, device=torch.device("cuda:0"), prune_n=0, prune_m=0):
    if "opt" in args.model:
        blocks = model.model.decoder.layers    
    else:
        blocks = model.model.layers
    ...
    for layer in layers:
        for name in layer:
            prunables.append(layer[name].weight.numel())
    layer_num = len(prunables)
    
    sparsity = args.sparsity_ratio * np.sum(prunables) / np.sum(prunables[1:])
    ratios = [sparsity for i in range(layer_num - 1)]
    ratios.insert(0, 0.0)
    
    prune_magnitude(args, model, tokenizer, device, ratios=ratios)
    
def prune_magnitude_uniform_plus_plus(args, model, tokenizer, device=torch.device("cuda:0"), prune_n=0, prune_m=0):
    if "opt" in args.model:
        blocks = model.model.decoder.layers    
    else:
        blocks = model.model.layers
    ...
    sparsity = args.sparsity_ratio * len(blocks) / (len(blocks) - 1)
    
    layer_per_block = int(layer_num / len(blocks))
    ratios = [sparsity for i in range(layer_num - layer_per_block)]
...
def prune_wanda_erk(args, model, tokenizer, device=torch.device("cuda:0"), prune_n=0, prune_m=0):
    # calculate ww-based layer
    all_layer_ratio = erk_sparsity(args, model, device)
    # magnitude pruning
    prune_wanda(args, model, tokenizer, device, ratios=all_layer_ratio)
    
def prune_wanda_erk_plus(args, model, tokenizer, device=torch.device("cuda:0"), prune_n=0, prune_m=0):
    # calculate ww-based layer
    all_layer_ratio = erk_sparsity_plus(args, model, device)
    # magnitude pruning
    prune_wanda(args, model, tokenizer, device, ratios=all_layer_ratio)
    
def prune_wanda_erk_plus_plus(args, model, tokenizer, device=torch.device("cuda:0"), prune_n=0, prune_m=0):
    # calculate ww-based layer
    all_layer_ratio = erk_sparsity_plus_plus(args, model, device)
    # magnitude pruning
    prune_wanda(args, model, tokenizer, device, ratios=all_layer_ratio)

def prune_wanda_uniform_plus(args, model, tokenizer, device=torch.device("cuda:0"), prune_n=0, prune_m=0):
    if "opt" in args.model:
        blocks = model.model.decoder.layers    
    else:
        blocks = model.model.layers
    ...
    prune_wanda(args, model, tokenizer, device, ratios=ratios)
    
def prune_wanda_uniform_plus_plus(args, model, tokenizer, device=torch.device("cuda:0"), prune_n=0, prune_m=0):
    if "opt" in args.model:
        blocks = model.model.decoder.layers    
    else:
        blocks = model.model.layers
...
# ==============
def prune_sparsegpt_erk(args, model, tokenizer, device=torch.device("cuda:0"), prune_n=0, prune_m=0):
    # calculate ww-based layer
    all_layer_ratio = erk_sparsity(args, model, device)
    # magnitude pruning
    prune_sparsegpt(args, model, tokenizer, device, ratios=all_layer_ratio)
    
def prune_sparsegpt_erk_plus(args, model, tokenizer, device=torch.device("cuda:0"), prune_n=0, prune_m=0):
    # calculate ww-based layer
    all_layer_ratio = erk_sparsity_plus(args, model, device)
    # magnitude pruning
    prune_sparsegpt(args, model, tokenizer, device, ratios=all_layer_ratio)
    
def prune_sparsegpt_erk_plus_plus(args, model, tokenizer, device=torch.device("cuda:0"), prune_n=0, prune_m=0):
    # calculate ww-based layer
    all_layer_ratio = erk_sparsity_plus_plus(args, model, device)
    # magnitude pruning
    prune_sparsegpt(args, model, tokenizer, device, ratios=all_layer_ratio)

def prune_sparsegpt_uniform_plus(args, model, tokenizer, device=torch.device("cuda:0"), prune_n=0, prune_m=0):
    if "opt" in args.model:
        blocks = model.model.decoder.layers    
    else:
        blocks = model.model.layers
    ...
    prune_sparsegpt(args, model, tokenizer, device, ratios=ratios)
    
def prune_sparsegpt_uniform_plus_plus(args, model, tokenizer, device=torch.device("cuda:0"), prune_n=0, prune_m=0):
    if "opt" in args.model:
        blocks = model.model.decoder.layers    
    else:
        blocks = model.model.layers
